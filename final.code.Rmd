...
```{r}
setwd("...")
final_combine=read.table("final_combine.txt",head=T)
length(final_combine)
DMA=final_combine$urxudma
MMA=final_combine$urxumma
inAs=final_combine$urxuas5+final_combine$urxuas3

final_combine$Re_total_arsenic=DMA+MMA+inAs
final_combine$inAsper=inAs/final_combine$Re_total_arsenic
final_combine$MMAper=MMA/final_combine$Re_total_arsenic
final_combine$DMAper=DMA/final_combine$Re_total_arsenic
final_combine$MMA_inAs=MMA/inAs
final_combine$DMA_MMA=DMA/MMA

write.table(final_combine,"final_calculate.txt",row.names=F,col.names=T)
...
```{r}
setwd("...")
UAS.dat = read.table("final_calculate.txt", head=T)
...
UAS.select2$folate_adjustsi_in2.75 = (log(UAS.select$folate_adjustsi)-2.7)*(log(UAS.select$folate_adjustsi)>2.7)
UAS.select2$folate_adjustsi_in3.25 = (log(UAS.select$folate_adjustsi)-3.25)*(log(UAS.select$folate_adjustsi)>3.25)
...
k3_25 = (knots2-3.25)*(knots2>3.25)

test.data2 = as.data.frame(cbind(rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),exp(knots2),rep(0,K2), k2_75, k3_25))
names(test.data2) = c("RIAGENDR","Age.cut","Race","povertytimes","BMI.cut","SMOKE","survey_year","folate_adjustsi","rDMA","folate_adjustsi_in2.75","folate_adjustsi_in3.25")
...
# result2 = lm(rDMA_MMA ~ RIAGENDR + Age.cut+ Race + povertytimes + BMI.cut + SMOKE +log(folate_adjustsi)+folate_adjustsi_in2.75 + folate_adjustsi_in3.25, data = UAS.select2, weights = wtsa2yr)
...
UAS.select2$folate_adjustsi_in2.75 = (log(UAS.select$folate_adjustsi)-2.7)*(log(UAS.select$folate_adjustsi)>2.7)
UAS.select2$folate_adjustsi_in3.25 = (log(UAS.select$folate_adjustsi)-3.4)*(log(UAS.select$folate_adjustsi)>3.4)

result2 = lm(rDMA ~  RIAGENDR +Age.cut+ Race + povertytimes + BMI.cut + SMOKE + survey_year+ log(folate_adjustsi)  , data = UAS.select2, weights = wtsa2yr/5)
summary(result2)
confint(result2)
# log(folate_adjustsi)
# +Creatinine +survey_year +log(lbxfolsi)
## test data for regression
K2 = 40
knots2 = seq(knots[1],knots[K-1],length.out = K2)
# test = outer(seq(knots[c(2,K-1)],length.out = K2),knots,"-")
...
test.data2 = as.data.frame(cbind(rep(1,K2),rep(4,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),exp(knots2),rep(0,K2), k2_75, k3_25))
names(test.data2) = c("RIAGENDR","Age.cut","Race","povertytimes","BMI.cut","SMOKE","survey_year","folate_adjustsi","rDMA","folate_adjustsi_in2.75","folate_adjustsi_in3.25")
...
test.data2 = as.data.frame(cbind(rep(1,K2),rep(4,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),rep(1,K2),exp(knots2)-1,rep(0,K2), k2_75, k3_25))
names(test.data2) = c("RIAGENDR","Age.cut","Race","povertytimes","BMI.cut","SMOKE","survey_year","folate_adjustsi","rDMA","folate_adjustsi_in2.75","folate_adjustsi_in3.25")
